<?php
 ...
 class User {
   ...
   public function __construct($username, $first_name, $second_name=null, $email,$profile=null,$id_user=null) {
      $this->username = $username;
      $this->first_name = $first_name;
      $this->second_name = $second_name;
      $this->email = $email;
 ...
 //Search user by ID
   public static function searchById($id_user){
       $connect = new Connect();
       $query = $connect->prepare('SELECT username, first_name, second_name, email, profile FROM ' . self::Table . ' WHERE id_client = :id_user');
       $query->bindParam(':id_user', $id_user);
       $query->execute();
       $response = $query->fetch();
       if($response){
          return new self($response['username'], $response['first_name'], $response['second_name'], $response['email'], $response['profile'], $id_user);
       }else{
          return false;
       }
    } 
